...
summary_data = transform.summarize_by_item(clean_data)
best_by_revenue = transform.find_top_entry(summary_data, "revenue")
best_by_units = transform.find_top_entry(summary_data, "units_sold")
...
display.print_item_table(summary_data)
print()
print(f"Top seller by revenue: {best_by_revenue['item']} (${best_by_revenue['revenue']:,.2f})")
print(f"Top seller by units:   {best_by_units['item']} ({best_by_units['units_sold']} units)")
